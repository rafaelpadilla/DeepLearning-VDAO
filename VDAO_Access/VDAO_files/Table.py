...
class Filters:
    ...
    def GetAllObjectsClasses(self):
        ...
        for o in self.videos:
            if (o.videoType == VideoType.WithObjects and
                o.object_class not in ret):
        ...
        for vid in self.videos: 
            if (vid.videoType == VideoType.WithObjects and vid.sourceTable.name == tableName):
                if objectClasses != None:
                    [ret.append(vid) for ocf in objectClasses if vid.object_class==ocf] #if object must be found
                else:
                    ret.append(vid)
        return ret
