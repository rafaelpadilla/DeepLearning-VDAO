...
class Player:
    def btnPlayPause_Clicked(self):
        ...
        if not hasattr(self, 'threadPlayOn'):
            ...
            self.videoPlayer1.cvVideo.set(cv2.CAP_PROP_POS_FRAMES,
                                          self.videoPlayer1.currentFrameNbr)
            self.videoPlayer2.cvVideo.set(cv2.CAP_PROP_POS_FRAMES,
                                          self.videoPlayer2.currentFrameNbr)
    ...
    def btnSelectFrame_Clicked(self):
        ...
        inputWindow = InputWindow(
            ...
            title="",
            minValue=1,
            maxValue=np.min([self.videoPlayer1.totalFrames, self.videoPlayer2.totalFrames]))
        inputWindow.Center(self.inputWindow)

    def btnRecord_Clicked(self):
        if self.writer is None:
            qte_files = len(glob.glob(os.path.join(self.PATH_TO_SAVE_RECORDINGS, '*rec*.mp4')))
            rec_path = os.path.join(self.PATH_TO_SAVE_RECORDINGS, ('%d_rec.mp4' % qte_files))
            self.writer = imageio.get_writer(rec_path, fps=60)
        else:
            self.writer.close()
            self.writer = None
    ...
    def PlayDiff(self, skippingFrame):
        if self.videoPlayer1.updatedFrames == self.videoPlayer2.updatedFrames or skippingFrame:
            ...
            # Se definido, cria arquivo de vídeo final
            if self.writer is not None:
                ...
                h = (15 * 2) + h1 + 100
                w = (w1 * 3) + (15 * 2)
                newImg = np.ones((h, w, 3), np.uint8)
                # Adiciona frame1
                newImg[0:h1, 0:w1, :] = self.frame1
                # Adiciona imagem diferenca
                newImg[0:h1, w1 + 15:(w1 * 2) + 15, :] = np.stack((diff, ) * 3, axis=2)
                # Adiciona frame2
                newImg[0:h1, (w1 * 2) + 30:(w1 * 3) + 30, :] = self.frame2
                self.writer.append_data(newImg)
    ...
    def __init__(self,
                 parent,
                 video1FilePath=None,
                 annotation1FilePath=None,
                 video2FilePath=None,
                 annotation2FilePath=None):
        self.root = parent
        # Set status of the player
        self.statusPlayer = StatusPlayer.NOT_STARTED
        self.eventPause = threading.Event()
        # Set writer to record
        self.writer = None
        self.PATH_TO_SAVE_RECORDINGS = 'C:\\vdao_teste\\'
        # current path
        ...
        self.imgBackwardsBeg = tk.PhotoImage(
            file=os.path.join(self.currentPath, 'aux_images', 'rewind_beg.png'))
        self.imgBackwards1 = tk.PhotoImage(
            file=os.path.join(self.currentPath, 'aux_images', 'rewind_1.png'))
        self.imgBackwards5 = tk.PhotoImage(
            file=os.path.join(self.currentPath, 'aux_images', 'rewind_5.png'))
        self.imgBackwards10 = tk.PhotoImage(
            file=os.path.join(self.currentPath, 'aux_images', 'rewind_10.png'))
        self.imgPlay = tk.PhotoImage(file=os.path.join(self.currentPath, 'aux_images', 'play.png'))
        self.imgPause = tk.PhotoImage(
            file=os.path.join(self.currentPath, 'aux_images', 'pause.png'))
        self.imgForwardEnd = tk.PhotoImage(
            file=os.path.join(self.currentPath, 'aux_images', 'forward_end.png'))
        self.imgForward1 = tk.PhotoImage(
            file=os.path.join(self.currentPath, 'aux_images', 'forward_1.png'))
        self.imgForward5 = tk.PhotoImage(
            file=os.path.join(self.currentPath, 'aux_images', 'forward_5.png'))
        self.imgForward10 = tk.PhotoImage(
            file=os.path.join(self.currentPath, 'aux_images', 'forward_10.png'))
