...
class VideoPlayer:
    ...
    def PlayOn(self):
        while self.cvVideo.isOpened():
            ...
            self.lblFrameNumber.configure(text="Frame: %d/%d" % (self.currentFrameNbr,self.totalFrames))
    ...
    def btnPlayPause_Clicked(self):
        ...
        if self.statusPlayer == StatusPlayer.NOT_STARTED: # it hasnt started yet
            self.callBack_PlayPauseBtn_Clicked(False, StatusPlayer.PLAYING) # disable buttons on Player and pass new action
            ...
            self.callBack_PlayPauseBtn_Clicked(True, StatusPlayer.PAUSED) # enable buttons on Player and pass new action
    ...
    def btnSelectFrame_Clicked(self):
        ...
        inputWindow = InputWindow(parent=self.inputWindow, eventClose=self.event_SetFrame, title="asd", minValue=1, maxValue=self.totalFrames)
    ...
    def UpdateVideoDetails(self, fileName, videoFilePath, annotationFilePath, currentFrameNbr):
        ...
        if self.totalFrames == 0: # Se vídeo não tem frames para tocar
            ...
            self.eventPause.set()
            self.threadPlayOn.start()
            self.callBack_PlayPauseBtn_Clicked(True, StatusPlayer.NOT_STARTED) # disable buttons on Player and pass new action
            self.ChangeNavButtonsStatus(True) # enable navigation buttons
            # Define starting frames
            self.GoToFrame(1)
    ...
    def __init__(self, parent, titleVideo, fileName, videoFilePath, annotationFilePath, currentFrameNbr, callback_FrameUpdated=None, callBack_PlayPauseBtn_Clicked=None, videoIsProcessed=False, callBack_EquateFrames=None):
        # Define variáveis
        ...
        lblTitle = tk.Label(self.lfVideo, text=titleVideo, font =("TkDefaultFont", 10, "bold", "underline"), justify=tk.CENTER, anchor=tk.W)
        lblTitle.pack()
        ...
        if self.videoIsProcessed == False:
            self.lblFileName = tk.Label(pnlFileName, text="File: "+fileName, justify=tk.LEFT, anchor=tk.W)
        else:
        ...
        if self.videoIsProcessed == False:
            ...
            self.chkAnnotations = tk.Checkbutton(pnlFrameNumber, text='show bounding boxes', variable=self.showBoundingBox, justify=tk.LEFT, anchor=tk.W, onvalue=1, offvalue=0, command=self.CheckBoxAnnotaion_Changed)
            pnlFrameNumber.add(self.chkAnnotations)
            self.showBoundingBox = False
            if not hasAnnotations:
                self.chkAnnotations.configure(state='disabled')
            self.lblFrameNumber = tk.Label(pnlFrameNumber, text="Frame: %d/%d" % (self.currentFrameNbr,self.totalFrames), justify=tk.LEFT, anchor=tk.W)
