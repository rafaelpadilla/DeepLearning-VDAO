...
class VideoPlayer:
    ...
    def Rewind(self, amountFrames):
        if amountFrames == None:
            ...
            self.currentFrameNbr -= amountFrames
        self.cvVideo.set(cv2.CAP_PROP_POS_FRAMES, self.currentFrameNbr-1)
        self.lblFrameNumber.configure(text="Frame: %d/%d" % (self.currentFrameNbr,self.totalFrames))
    ...
    def MoveForward(self, amountFrames):
        if amountFrames == None:
            ...
            self.currentFrameNbr = self.totalFrames
        else:
            self.currentFrameNbr += amountFrames
        self.cvVideo.set(cv2.CAP_PROP_POS_FRAMES, self.currentFrameNbr-1)
        self.lblFrameNumber.configure(text="Frame: %d/%d" % (self.currentFrameNbr,self.totalFrames))
    ...
    def __init__(self, parent, titleVideo, fileName, videoFilePath, annotationFilePath, currentFrameNbr, callback_FrameUpdated=None, callBack_PlayPauseBtn_Clicked=None, videoIsProcessed=False):
        # Define variáveis
