...
class ObjectDatabase:
    ...
    @staticmethod
    def blendImageAndBackground(image,
                                mask,
                                background,
                                method=BlendingMethod.BrunosMethod,
                                xIni=0,
                                yIni=0,
                                scaleFactor=1,
                                rotAngle=0,
        ...
        if method == BlendingMethod.BrunosMethod:
            return ObjectDatabase.blendImageAndBackground_BrunosMethod(
                image, mask, background, xIni, yIni, scaleFactor, rotAngle, flipHor, iteracao1,
                iteracao2)
        elif method == BlendingMethod.RafaelsMethod:
            return ObjectDatabase.blendImageAndBackground_RafaelsMethod(
                image, mask, background, xIni, yIni, scaleFactor, rotAngle, flipHor, iteracao1,
                iteracao2)
        elif method == BlendingMethod.KeepBlurLevel:
            return ObjectDatabase.blendImageAndBackground_KeepBlurLevel(
                image, mask, background, xIni, yIni, scaleFactor, rotAngle, flipHor,
                saveIntermediateImages, folderSaveIntermImages)
        elif method == BlendingMethod.OnlyBlend:
            return ObjectDatabase.blendImageAndBackground_OnlyBlend(image, mask, background, xIni,
                                                                    yIni, scaleFactor, rotAngle,
                                                                    flipHor)

    @staticmethod
    def blendImageAndBackground_OnlyBlend(image, mask, background, xIni, yIni, scaleFactor,
                                          rotAngle, flipHor):
        # A paths were passed instead of loaded images
    ...
    def blendImageAndBackground_KeepBlurLevel(image,
        ...
        if saveIntermediateImages:
            ...
            fh.write('\nDistance blur (backgroundWithShoeNoTreatment and roiBackground): %f' %
                     utils.euclideanDistance(blurImageNoTreatment, blurLevelReference))
            ...
            cv2.imwrite(os.path.join(fol, 'imgMaskBlended.png'), imgMaskBlended,
                        [cv2.IMWRITE_PNG_COMPRESSION, 0])
            cv2.imwrite(os.path.join(fol, 'mask_larger_1.png'), mask_larger_1_bin * 255,
                        [cv2.IMWRITE_PNG_COMPRESSION, 0])
            cv2.imwrite(os.path.join(fol, 'layer_1.png'), layer_1 * 255,
                        [cv2.IMWRITE_PNG_COMPRESSION, 0])
            cv2.imwrite(os.path.join(fol, 'layer_2.png'), layer_2_bin * 255,
                        [cv2.IMWRITE_PNG_COMPRESSION, 0])
            cv2.imwrite(os.path.join(fol, 'layer_2_bin_inv.png'), layer_2_bin_inv * 255,
                        [cv2.IMWRITE_PNG_COMPRESSION, 0])
            ...
            cv2.imwrite(os.path.join(fol, 'backgroundWithShoeNoTreatment.png'),
                        backgroundWithShoeNoTreatment, [cv2.IMWRITE_PNG_COMPRESSION, 0])
            cv2.imwrite(os.path.join(fol, 'layer_1_with_background.png'), layer_1_with_background,
                        [cv2.IMWRITE_PNG_COMPRESSION, 0])
            cv2.imwrite(os.path.join(fol, 'layer_2_with_background.png'), layer_2_with_background,
                        [cv2.IMWRITE_PNG_COMPRESSION, 0])
            cv2.imwrite(os.path.join(fol, 'shoes_background.png'), shoes_background,
                        [cv2.IMWRITE_PNG_COMPRESSION, 0])
            cv2.imwrite(os.path.join(fol, 'backgroundWithLarge1ShoeMask.png'),
                        backgroundWithLarge1ShoeMask, [cv2.IMWRITE_PNG_COMPRESSION, 0])
        ...
        while True:
            ...
            if saveIntermediateImages:
                ...
                fh.write('\nDistance blur (%d_finalImage and roiBackground): %f' %
                         (iteration, distLoop))
                cv2.imwrite(os.path.join(fol, '%d_shoes_background.png' % iteration),
                            shoes_background, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                cv2.imwrite(os.path.join(fol, '%d_blurredShoeBackground.png' % iteration),
                            blurredShoeBackground, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                cv2.imwrite(os.path.join(fol, '%d_ghostyBackgroundWithShoe.png' % iteration),
                            ghostyBackgroundWithShoe, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                cv2.imwrite(os.path.join(fol, '%d_prebackgroundWithShoe.png' % iteration),
                            prebackgroundWithShoe, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                cv2.imwrite(os.path.join(fol, '%d_finalImage.png' % iteration), finalImage,
                            [cv2.IMWRITE_PNG_COMPRESSION, 0])
    ...
    # Using my method
    @staticmethod
    def blendImageAndBackground_RafaelsMethod(image,
                                              mask,
                                              background,
                                              xIni=0,
                                              yIni=0,
        ...
        dist_transf = cv2.distanceTransform(src=mask_diff_erode,
        ...
        guide[yIni:h + yIni, xIni:w + xIni, 0:3] = cv2.merge(
            (final_mask / 255, final_mask / 255, final_mask / 255))
    ...
    def blendImageAndBackground_BrunosMethod(image,
                                             mask,
                                             background,
                                             xIni=0,
                                             yIni=0,
        ...
        img_mask_bin_dilate = cv2.dilate(src=img_mask_bin,
                                         kernel=STREL_KERNEL,
                                         iterations=iteracao1)
        tresh_val, img_mask_bin_inv_dilate = cv2.threshold(img_mask_bin_dilate, 127, 255,
                                                           cv2.THRESH_BINARY_INV)
        img_mask_bin_backg_dilate = cv2.dilate(src=img_mask_bin_dilate,
                                               kernel=STREL_KERNEL,
                                               iterations=iteracao2)
        ...
        img_mask_bin_dilate = np.concatenate(
            (img_mask_bin_dilate, img_mask_bin_dilate, img_mask_bin_dilate), axis=2)
        img_mask_bin_inv_dilate = np.expand_dims(img_mask_bin_inv_dilate, axis=2)
        img_mask_bin_inv_dilate = np.concatenate(
            (img_mask_bin_inv_dilate, img_mask_bin_inv_dilate, img_mask_bin_inv_dilate), axis=2)
        img_mask_bin_backg_dilate = np.expand_dims(img_mask_bin_backg_dilate, axis=2)
        img_mask_bin_backg_dilate = np.concatenate(
            (img_mask_bin_backg_dilate, img_mask_bin_backg_dilate, img_mask_bin_backg_dilate),
            axis=2)
        img_mask_bin_backg_inv_dilate = np.expand_dims(img_mask_bin_backg_inv_dilate, axis=2)
        img_mask_bin_backg_inv_dilate = np.concatenate(
            (img_mask_bin_backg_inv_dilate, img_mask_bin_backg_inv_dilate,
             img_mask_bin_backg_inv_dilate),
            axis=2)
        # GENERATES NEW MASKS
        ...
        img_mask_bin_dif_dilate = np.subtract(img_mask_bin_inv_dilate,
                                              img_mask_bin_backg_inv_dilate)
        ...
        img_dist_transf = cv2.distanceTransform(src=img_mask_bin_inv_dilate_ORG,
                                                distanceType=cv2.DIST_L2,
                                                maskSize=5)
        img_dist_transf = np.expand_dims(img_dist_transf, axis=2)
        img_dist_transf = np.concatenate((img_dist_transf, img_dist_transf, img_dist_transf),
                                         axis=2)
        ...
        img_blur_border = np.add(np.multiply(img_weight_foreg, img_mask_new),
                                 np.multiply(img_weight_backg, img_mask_new_backg))
        # Find bounding box where the object will be inserted
    ...
    @staticmethod
    def generateImages(imageDetailsFile,
                       imagesFolder,
                       maskFolder,
                       backgroundFolder,
                       outputDir,
                       maskPrefix='',
                       blendingMethod=BlendingMethod.OnlyBlend):
        skipArray = []
